...
// @updateURL   https://raw.githubusercontent.com/deltabravozulu/usefulUserScripts/main/amazonReviewWall/amazonReviewWallNewTab.user.js
// @downloadURL https://raw.githubusercontent.com/deltabravozulu/usefulUserScripts/main/amazonReviewWall/amazonReviewWallNewTab.user.js
...
async function theStuff() {
    ...
    function appendHTML() {
        var wrapper = newWindow.document.createElement("body");
        wrapper.innerHTML =
'\
<style>\
html,body {\
...
';
        newWindow.document.appendChild(wrapper);
    }
    await sleep(1000);
    ...
    newWindow.document.body.innerHTML = 'howdy';
    var newDoc = newWindow.document.open("text/html", "replace");
    appendHTML();
    await sleep(1000);
    newWindow.document.getElementById("zoomwall").innerHTML = text;
    await sleep(1000);
    var zoomwall = {
        create: function (blocks, enableKeys) {
            ...
            if (enableKeys) {
                ...
                newWindow.document.addEventListener("keydown", keyPager);
            }
        },
        ...
        expand: function (block) {
            ...
            // parent dimensions
            var parentStyle = newWindow.getComputedStyle(block.parentNode);
            var parentWidth = parseInt(parentStyle.width, 10);
            var parentHeight = parseInt(parentStyle.height, 10);
            var parentTop = block.parentNode.getBoundingClientRect().top;
            // block dimensions
            var blockStyle = newWindow.getComputedStyle(block);
            var blockWidth = parseInt(blockStyle.width, 10);
            var blockHeight = parseInt(blockStyle.height, 10);
            // determine maximum height
            var targetHeight = newWindow.innerHeight;
            if (parentHeight < newWindow.innerHeight) {
    ...
    await sleep(1000);
    zoomwall.create(newWindow.document.getElementById("zoomwall"), true);
    await sleep(1000);
    newDoc.close();
}
  ///////////////////////////////////////
